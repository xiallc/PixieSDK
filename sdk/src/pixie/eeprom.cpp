...
static const std::vector<eeprom_format> formats =
{
    { { 0x0a, 0x0f }, 0 },
    { { 0x0f, 0x0f }, 1 },
    { { 0x48, 0xff }, 2 }
...
static const std::vector<v1_config> v1_configs =
{
...
{
    { tag::model,
      { tag::model,       "model",       string, 16, true,  false } },
    { tag::serial_num,
      { tag::serial_num,  "serial_num",  num32,  4,  true,  false } },
    { tag::revision,
      { tag::revision,    "revision",    num8,   1,  true,  false } },
    { tag::mod_strike,
      { tag::mod_strike,  "mod-strike",  num8,   1,  true,  false } },
    { tag::index,
      { tag::index,       "index",       num16,  2,  false, false } },
    { tag::size,
      { tag::size,        "size",        num16,  2,  false, false } },
    { tag::format_ver,
      { tag::format_ver,  "format ver",  num8,   1,  false, false } },
    { tag::adc_bits,
      { tag::adc_bits,    "adc_bits",    num8,   1,  false, false } },
    { tag::adc_msps,
      { tag::adc_msps,    "adc msps",    num16,  2,  false, false } },
    { tag::adc_clk_div,
      { tag::adc_clk_div, "adc clk div", num8,   2,  false, false } },
    { tag::fpga_clk,
      { tag::fpga_clk,    "fpga clk",    num8,   2,  false, false } },
    { tag::end,
      { tag::end,         "end",         null,   0,  true,  false } }
};
...
{
    { 0, 4, hw::config(14,  75, 2,  75 / 2), "DB01" },
...
eeprom::process()
{
    ...
    if (crc.value != hdr.crc) {
        ...
        if (config.adc_bits == 0) {
            ...
            oss << "serial number not supported, not format map: "
                << serial_num;
            throw error(error::code::device_eeprom_failure,
                        oss.str());
        }
    ...
    if (mc == rev_max_channels.end()) {
        throw error(error::code::device_eeprom_failure,
                    "no rev to max channels mapping: " +
...
std::string
eeprom::get_string(const tag key, size_t count)
{
...
int
eeprom::get_number(const tag key, size_t count)
{
    auto& desc = lookup(key);
    size_t offset = find(key, count);
    int value = 0;
...
double
eeprom::get_float(const tag key, size_t count)
{
    auto& desc = lookup(key);
    if (desc.type != string) {
        throw_bad_tag_type(key);
...
mac_addr
eeprom::get_mac(const tag key, size_t count)
{
    auto& desc = lookup(key);
    if (desc.type != string) {
        throw_bad_tag_type(key);
...
ipv4_addr
eeprom::get_ipv4(const tag key, size_t count)
{
    auto& desc = lookup(key);
    if (desc.type != string) {
        throw_bad_tag_type(key);
...
size_t
eeprom::find(const tag key, size_t count) const
{
    if (format >= 2) {
    ...
    std::ostringstream oss;
    oss << "tag not found: " << std::hex << int(key)
        << " count: " << count;
    throw error(error::code::device_eeprom_not_found,
                oss.str());
}

const element_decs&
eeprom::lookup(const tag key) const
{
...
void
eeprom::throw_bad_tag_type(const tag key)
{
    std::ostringstream oss;
    oss << "invalid tag type: " << std::hex << int(key);
    throw error(error::code::device_eeprom_bad_type,
                oss.str());
}
