...
void report(pixie::module::module& module, pixie::format::json& out) {
    if (module.present()) {
        ...
        out["fifo"]["buffers"] = module.fifo_buffers;
        out["fifo"]["run_wait"] = module.fifo_run_wait_usecs.load();
        out["fifo"]["idle_wait"] = module.fifo_idle_wait_usecs.load();
        out["fifo"]["hold"] = module.fifo_hold_usecs.load();
        out["fifo"]["dma_trig"] = module.fifo_dma_trigger_level.load();
        out["fifo"]["bandwidth"] = module.fifo_bandwidth.load();
