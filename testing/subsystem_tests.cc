...
int test_analogfrontend (unsigned short nummodules)
{
	...
	{
		sprintf(ErrMSG, "*ERROR* (test_analogfrontend): nummodules %d exceeds maximum allowed number of modules %d", nummodules, PRESET_MAX_MODULES);
		Pixie_Print_MSG(ErrMSG);
		return(-1);
	}
	...
	for(k=0; k<nummodules; k++)
	{
		...
		if(retval < 0)
		{
			sprintf(ErrMSG, "*ERROR* (test_analogfrontend): Could not read I2C-EEPROM in Module=%ld; retval=%d", k, retval);
			Pixie_Print_MSG(ErrMSG);
			return(-2);
		}
	...
	for(k=0; k<65535; k++)
	{
		...
		for(m=0; m<nummodules; m++)
		{
			for(n=0; n<16; n++)
			{
				...
				if(retval < 0)
				{
					sprintf(ErrMSG, "*ERROR* (test_analogfrontend): Pixie16WriteSglChanPar failed in mod %d, chan %d, retval=%d", m, n, retval);
					Pixie_Print_MSG(ErrMSG);
		...
		for(m=0; m<nummodules; m++)
		{
			...
			if(retval < 0)
			{
				sprintf(ErrMSG, "*ERROR* (test_analogfrontend): Pixie16AcquireADCTrace failed in mod %d, retval=%d", m, retval);
				Pixie_Print_MSG(ErrMSG);
				// Close all data files
				for(i=0; i<nummodules; i++)
				{
			...
			for(n=0; n<16; n++)
			{
				...
				if(retval < 0)
				{
					sprintf(ErrMSG, "*ERROR* (test_analogfrontend): Pixie16ReadSglChanADCTrace failed in mod %d, chan %d, retval=%d", m, n, retval);
					Pixie_Print_MSG(ErrMSG);
					// Close all data files
					for(i=0; i<nummodules; i++)
					{
				...
				retval = Pixie16IMbufferIO((unsigned int *)&offsetdac, 1, (unsigned long)(OffsetDAC_Address[m] + n), MOD_READ, m);
				if(retval < 0)
				{
					sprintf(ErrMSG, "*ERROR* (test_analogfrontend): Pixie16IMbufferIO failed in mod %d, chan %d, retval=%d", m, n, retval);
					Pixie_Print_MSG(ErrMSG);
					// Close all data files
					for(i=0; i<nummodules; i++)
					{
