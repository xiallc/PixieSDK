...
Begin VB.Form showDSPMemBlock0 
   Caption         =   "Show DSP Memory Block 0 (0x00049180 - 0x0004FFFF)"
   ClientHeight    =   9465
   ClientLeft      =   60
   ClientTop       =   450
   ClientWidth     =   7815
   ...
   Begin vbAcceleratorSGrid6.vbalGrid vbalGrid1 
      ...
      DisableIcons    =   -1  'True
   End
   Begin VB.CommandButton CloseDSPMemBlock0 
      Caption         =   "Close"
      Height          =   615
      Left            =   6120
      TabIndex        =   1
      ...
      Width           =   1575
   End
   Begin VB.CommandButton RefreshDSPMemBlock0 
      Caption         =   "Refresh"
      BeginProperty Font 
         Name            =   "MS Sans Serif"
         Size            =   8.25
      ...
      Width           =   1575
   End
   Begin CWUIControlsLib.CWNumEdit showDSPMemBlock0ModNumEdit 
      Height          =   495
      Left            =   1680
      TabIndex        =   5
      Top             =   8640
      ...
      Owner_5         =   1
   End
   Begin VB.Label ShowDSPMemBlock0ModNumLabel 
   ...
   End
End
Attribute VB_Name = "showDSPMemBlock0"
Attribute VB_GlobalNameSpace = False
Attribute VB_Creatable = False
Attribute VB_PredeclaredId = True
Attribute VB_Exposed = False
Private Sub CloseDSPMemBlock0_Click()
    showDSPMemBlock0.Hide
End Sub
...
Private Sub Form_Load()
    ...
    showDSPMemBlock0ModNumEdit.Maximum = Number_Modules_Total - 1
    
    ' Load DSP list and the grids - use default module 0
    Call Load_DSPlsts0(0)
...
Private Sub RefreshDSPMemBlock0_Click()
    ...
    ModuleNumber = CInt(showDSPMemBlock0ModNumEdit.Text)
    
    ' Calculate the starting address of the DSP IOparams
    startaddr = DSP_PMDATA_ADDR - DSP_IMBUFFER_ADDR
    
    'Calculate the number of words to read from the DSP
    NumWords = &H50000 - DSP_PMDATA_ADDR
    
    ' Check if we are using online mode
    If IsOfflineOperation = 0 Then
    
        ' Read module parameters
        retval = Pixie16IMbufferIO(DSPMemBlock0(startaddr), NumWords, DSP_PMDATA_ADDR, MOD_READ, ModuleNumber)
        If retval < 0 Then
            StrTxt = "RefreshDSPMemBlock0: Pixie16IMbufferIO for module # " & ModuleNumber & " failed, retval = " & retval
    ...
    Else
        ...
        For index = startaddr To startaddr + NumWords - 1
    ...
    With vbalGrid1
        ...
        For i = 1 To .Rows - 1
            If DSPMemBlock0(startaddr + i - 1) < 0 Then
                If DSPMemBlk0IsHexadecimal = True Then
                    .CellText(i, 3) = ToHex(CDbl(DSPMemBlock0(startaddr + i - 1)) + 4294967296#)
                Else
                    .CellText(i, 3) = CDbl(DSPMemBlock0(startaddr + i - 1)) + 4294967296#
                End If
            Else
                If DSPMemBlk0IsHexadecimal = True Then
                    .CellText(i, 3) = ToHex(CDbl(DSPMemBlock0(startaddr + i - 1)))
                Else
                    .CellText(i, 3) = CDbl(DSPMemBlock0(startaddr + i - 1))
                End If
            End If
...
Private Sub Load_DSPlsts0(ModNum As Integer)
    Dim X As Double
    ...
    With vbalGrid1
        ...
        ' Add the columns
        .AddColumn "colAddr", "Address", ecgHdrTextALignRight, , 100, True, True, , , , False, , False
        .AddColumn "colVal", "DSP Val", ecgHdrTextALignRight, , 200, True, True
        .AddColumn "colPar", "DSP Par", ecgHdrTextALignRight, , 150, True, True
        
        .Rows = &H50000 - DSP_PMDATA_ADDR
        .DefaultRowHeight = 30
        ...
        Do While 1
            ...
            Else
                ...
                ' Check if this DSP variable has mutliple entries, e.g. channel dependent parameter
                addrdiff = startingaddr - currentaddr
                If addrdiff > 1 Then
                    ' Include entry number in the DSP variable name
                    X = 0
                    .CellText(currentaddr - startingaddr + 1 + X, 2) = Right(LineStr1, (Len(LineStr1) - 12)) & "[" & Format(X, "00") & "]"
            ...
            ' Check if we are done with memory block 0 parameters
            If currentaddr >= &H50000 Then
                Exit Do
            End If
        Loop
       
        For X = 1 To .Rows
            .CellText(X, 1) = "0x" & ToHex(X - 1 + 299392)
...
Private Sub showDSPMemBlock0_ModNumEdit_ValueChanged(Value As Variant, PreviousValue As Variant, ByVal OutOfRange As Boolean)
    ' re-load DSP list
    Call Load_DSPlsts0(showDSPMemBlock0_ModNumEdit.Value)
End Sub
