...
TEST_SUITE("Crate: modules") {
   TEST_CASE("SETUP") {
    ...
    TEST_CASE("backplane") {
        ...
        CHECK_NOTHROW(crate.initialize());
        CHECK_NOTHROW(crate.probe());
        /* wired-or trigger pullup */
        CHECK_NOTHROW(crate[0].write(module_param::module_csrb, 1 << hw::bit::MODCSRB_CPLDPULLUP));
        CHECK_THROWS_WITH_AS(
            crate[2].write(module_param::module_csrb, 1 << hw::bit::MODCSRB_CPLDPULLUP),
            "module: num=2,slot=10: wired-or trigger pullups leader role already taken: 0", crate_error);
        CHECK_NOTHROW(crate[0].write(module_param::module_csrb, 0));
        CHECK_NOTHROW(crate[2].write(module_param::module_csrb, 1 << hw::bit::MODCSRB_CPLDPULLUP));
        ...
        CHECK_NOTHROW(crate[2].write(module_param::module_csrb, 0));
        /* run leader */
        CHECK_NOTHROW(crate[0].write(module_param::module_csrb, 1 << hw::bit::MODCSRB_CHASSISMASTER));
        CHECK_THROWS_WITH_AS(
        ...
        CHECK_NOTHROW(crate[0].write(module_param::module_csrb, 0));
        CHECK_NOTHROW(crate[2].write(module_param::module_csrb, 1 << hw::bit::MODCSRB_CHASSISMASTER));
        CHECK_NOTHROW(crate[2].write(module_param::module_csrb, 0));
        /* no synch wait */
