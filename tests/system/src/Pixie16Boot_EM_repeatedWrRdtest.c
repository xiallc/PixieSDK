...
int main(int argc, char *argv[])
{
	...
	unsigned int count;
	unsigned short modnum, numtries, emwrites;
	...
	do
	{
		...
		{	
			...
			for(modnum = 0; modnum < NumModules; modnum ++)
			{
				...
				{
					retval = Pixie_Main_Memory_IO(wr_data, 65536*emwrites, 65536, MOD_WRITE, modnum);
					if (retval < 0)
					{
						sprintf (ErrMSG, "*ERROR* Pixie16_Main_Memory_IO WRITE failed in module %d, retval = %d", modnum, retval);
						Pixie_Print_MSG (ErrMSG);
					}
					...
					for(count = 0; count < 65536; count ++)
					{
						...
						{
							sprintf (ErrMSG, "RD_WR data mismatch in module %d, rd_data=0x%x, wr_data=0x%x, em address=%x", modnum, rd_data[count], wr_data[count], count+65536*emwrites);
							Pixie_Print_MSG (ErrMSG);
							rd_wr_errors ++;
